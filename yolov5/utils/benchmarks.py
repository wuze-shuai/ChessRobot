    ...
    for i, (name, f, suffix, gpu) in formats.iterrows():  # index, (name, file, suffix, gpu-capable)
        try:
            ...
            if f == '-':
                ...
                w = export.start_detect_cmd(weights=weights, imgsz=[imgsz], include=[f], device=device, half=half)[-1]  # all others
            assert suffix in str(w), 'export failed'
            result = val.start_detect_cmd(data, w, batch_size, imgsz, plots=False, device=device, task='benchmark', half=half)
