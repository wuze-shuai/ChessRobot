...
def run_yolo_detection(image_dir, save_dir, weights, data, img_size=640, conf_thres=0.25, iou_thres=0.45, device=""):
    ...
    Args:
        ...
        save_dir (str): 保存检测结果的目录路径。
        weights (str): YOLOv5模型权重文件路径。
        data (str): 数据集配置文件（.yaml）路径。
        img_size (int, optional): 推理图像大小。默认为640。
        conf_thres (float, optional): 置信度阈值。默认为0.25。
        iou_thres (float, optional): NMS IOU阈值。默认为0.45。
        device (str, optional): 使用的设备，例如"0"或"cpu"。默认为空字符串，自动选择设备。
    """
    # 创建保存目录
    ...
    for image_file in image_files:
        ...
        if len(det):
            ...
            det[:, :4] = scale_coords(im_pil.shape[2:], det[:, :4], image.shape).round()
...
if __name__ == "__main__":
    image_dir = r"E:\13project\yolov5\data\images\black_white"  # 图像目录
    save_dir = r"E:\13project\yolov5\data\labels\black_white"  # 保存检测结果的目录
    weights = r"E:\13project\yolov5\runs\train\exp_best\weights\best.pt"  # 模型权重文件路径
    data = r"E:\13project\yolov5\data\myData.yaml"  # 数据集配置文件路径

    # 读取 YAML 并生成 classes.txt
    classes_path = Path(save_dir) / 'classes.txt'
    load_classes_from_yaml(data, classes_path)
